...
class DataLoader:
    def __init__(self,sents,batch_size=1,fix_length=500,init_token='<bos>',eos_token='<bos>',stop_words=''):
        ...
        self.TEXT = data.Field(sequential=True, init_token=self.init_token, stop_words=self.stop_words,
                               fix_length=self.fix_length)
        self.LABEL = data.Field(sequential=True, init_token=self.init_token, stop_words=self.stop_words,
                               fix_length=self.fix_length)
...
if __name__ == '__main__':
    ...
    sents = w.sents
    d = DataLoader(sents=sents,batch_size=32)
    words = '.'.join(jieba.cut('去北京玩'))
    words = words.split('.')
    word_tensor = d.create_one_data(words)